...
def connection(user,pwd,host,port,db):
    engine = create_engine('mssql+pyodbc://{user}:{pwd}@{host}:{port}/{db}?driver=SQL+Server'.format(user= user,pwd = pwd,host = host,port = port,db = db))
...
def insert_into(co):
    if co[-4:] == "xlsx" and "~$" not in co:
        dic = pd.read_excel(path + "\\" + point_date + "\\" + co,
                            sheet_name=["日报", "充值提现", "收支调整", "费用", "冻结", "借入借出", "借出台湾", "余额表-银行", "余额表-三方"]
                            , skiprows=[0])
        for key, values in dic.items():
            ...
            if key == "充值提现" or key == "收支调整" or key == "费用" or key == "冻结" or key == "借入借出" or key == "借出台湾":
                ...
                try:
                    ...
                    values['其'] = dt.datetime.strptime(point_date, "%Y-%m")
                    for column in ['余额差', '后台收入', '掉补单', '支出手续费', '支出笔数', '支出金额', '收入手续费', '收入笔数', '收入金额', '期初金额', '期末金额']:
                        try:
                            values[column] = values[column].mask(values[column].str.contains('.', na=False), None)
                        except:
                            pass
                    values = values.rename(columns = {"日期":"日其","期末金额":"其末金额","期初金额":"其初金额","三方名/银行名":"三方银行名","掉补单":"调补单"})
...
def main():
    global path,db_info,engine, con, cursor, point_date, log_fail, bal_fail, day_fail
    path = r"Z:\02-帳務"
    db_info = db_info()
